...
ChronosGraph is a research system, not a chatbot. Every answer must be traceable:
...
uv run chronos migrate       # apply db/migrations in order
uv run chronos doctor        # environment + readiness report
...
| Command | Purpose |
|---|---|
| `chronos doctor` | Environment and readiness report. Changes nothing. |
| `chronos migrate` | Apply pending SQL migrations. |
| `chronos check` | Run the research invariant checks in `db/checks/invariants.sql`. |
| `chronos sources` | Show the source registry and its human-owned approval state. |
| `chronos config` | Print effective configuration with secrets redacted. |
...
src/chronosgraph/      the research code
